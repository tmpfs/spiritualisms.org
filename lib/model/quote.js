...
function getStar(opts, cb) {
  ...
  server.doc.head({id: opts.id}, function(err) {
    ...
    client.get('star://' + opts.id, function(err, res) {
      if(err) {
        return cb(err); 
      }
...
function addStar(opts, cb) {
  ...
  server.doc.head({id: opts.id}, function(err) {
    ...
    client.incr('star://' + opts.id, function(err, res) {
      if(err) {
        return cb(err); 
      }
      var o = {};
...
function removeStar(opts, cb) {
  ...
  server.doc.head({id: opts.id}, function(err) {
    ...
    client.decr('star://' + opts.id, function(err, res) {
      if(err) {
        return cb(err); 
      }
      var o = {};
...
/**
 *  Decrement the star counter for multiple quoteis, the quote documents
 *  must exist.
 *
 *  @param opts {Object} Request options.
 *  @param cb {Function} Callback function.
...
function removeStars(opts, cb) {
  ...
  this.list(vopts, function(err, res, body) {
    ...
    multi.exec(function(err, res) {
      ...
      var o = {};
      valid.forEach(function(id, index) {
        o[id] = parseInt(res[index]);
        if(o[id] < 0) {
          o[id] = 0; 
        }
      })
